AverageExpressionSE_v2 <- function (object, assays = 'RNA', features = NULL, return.seurat = FALSE,
    ...
    verbose = TRUE, ...)
{
    ...
    for (i in 1:length(x = assays)) {
        ...
        for (j in levels(x = Idents(object))) {
            ...
            if (verbose) {
                message(paste("Finished calculating standard error for cluster", j))
            }
            if (i == 1) {
