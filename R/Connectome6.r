#' Connectome Version 6
#'
#' Creates a connectomic edgelist from a Seurat 2.0 object
#'
...
#' @export
Connectome6 <- function(object,use.supported_known = T,use.supported = F,use.all = F,...){
  library(Seurat)
  library(dplyr)
  library(tibble)
  library(plotrix)
...
# Create averages and other relevant cluster-wise metrics, of only these GOI, convert genes to uppercase
cluster.avgs <- AverageExpressionLog(object,genes.use = genes.use,...)
cluster.avgs.raw <- AverageExpression(object,genes.use = genes.use,use.raw = T,...)
cluster.avgs.scale <- AverageExpression(object,genes.use = genes.use,use.scale = T,...)
cluster.avgs.SE <- AverageExpressionSE(object,genes.use = genes.use,...)
cluster.avgs.raw.SE <- AverageExpressionSE(object,genes.use = genes.use,use.raw = T,...)
cluster.avgs.scale.SE <- AverageExpressionSE(object,genes.use = genes.use,use.scale = T,...)
cluster.pcts <- PercentExpression(object,genes.use = genes.use,use.raw = T,...) #percent cells in cluster expressing greater than zero
...
row.names(cluster.avgs) <- toupper(row.names(cluster.avgs))
row.names(cluster.avgs.raw) <- toupper(row.names(cluster.avgs.raw))
row.names(cluster.avgs.scale) <- toupper(row.names(cluster.avgs.scale))
row.names(cluster.avgs.SE) <- toupper(row.names(cluster.avgs.SE))
row.names(cluster.avgs.raw.SE) <- toupper(row.names(cluster.avgs.raw.SE))
row.names(cluster.avgs.scale.SE) <- toupper(row.names(cluster.avgs.scale.SE))
row.names(cluster.pcts) <- toupper(row.names(cluster.pcts))
