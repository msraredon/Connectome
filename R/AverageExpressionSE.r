AverageExpressionSE <- function (object, genes.use = NULL, return.seurat = FALSE, add.ident = NULL,
                                 use.scale = FALSE, use.raw = FALSE, show.progress = TRUE,
                                 ...)
{
    ...
    slot.use <- "data"
    fxn.average <- function(x) std.error(expm1(x))
    if (use.scale) {
        slot.use <- "scale.data"
        fxn.average <- std.error
    }
    if (use.raw) {
        slot.use <- "raw.data"
        fxn.average <- std.error
    ...
    for (i in 1:length(x = assays.use)) {
        ...
        for (j in levels(x = object@ident)) {
            ...
            if (show.progress) {
                print(paste0("Finished calculating standard error ", assays.use[i],
                             " for cluster ", j))
            }
            if (i == 1) {
