AverageExpressionLog <- function (object, genes.use = NULL, return.seurat = FALSE, add.ident = NULL,
    ...
    ...)
{
    ...
    for (i in 1:length(x = assays.use)) {
        ...
        for (j in levels(x = object@ident)) {
            ...
            if (show.progress) {
                message(paste("Finished averaging", assays.use[i],
                  "for cluster", j))
            }
            if (i == 1) {
